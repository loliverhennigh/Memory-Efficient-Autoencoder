...
def standard_res_encoder(inputs, filter_size=8, nr_downsamples=5, nr_residual=2, nonlinearity="concat_elu", gated=True):
  x_i = inputs
  nonlinearity = set_nonlinearity(nonlinearity)

  for i in xrange(nr_downsamples):
    filter_size = filter_size*(pow(2,i))
...
def standard_res_decoding(inputs, final_size, filter_size=8, nr_downsamples=5, nr_residual=2, nonlinearity="concat_elu", gated=True):
  y_i = inputs
  nonlinearity = set_nonlinearity(nonlinearity)

  for i in xrange(nr_downsamples-1):
    filter_size = filter_size*pow(2,nr_downsamples-i-1)
